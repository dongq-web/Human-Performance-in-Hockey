rpi <- read.csv('possesion_event_d1_rpi.csv', fileEncoding="UTF-8-BOM")
rpia <- rpi$RPIA
rpib <- rpi$RPIB
rpiduration <- rpi$duration
oppo <- read.csv('possesion_event_d1_oppo.csv', fileEncoding="UTF-8-BOM")
oppoa <- oppo$OPPOA
oppob <- oppo$OPPOB
oppoduration <- oppo$duration
shot_for <- read.csv('possesion_event_d1_for.csv', fileEncoding="UTF-8-BOM")
forchance <- matrix(shot_for$Chance)
shot_for <- shot_for$Shot_for
shot_against <- read.csv('possesion_event_d1_against.csv', fileEncoding="UTF-8-BOM")
againstchance <- matrix(shot_against$Chance)
shot_against <- shot_against$Shot_against

formatrix <- matrix(nrow = 11, ncol = 2)
f <- 0
...
againstmatrix <- matrix(nrow = 13, ncol = 2)
f <- 0
