rpi <- read.csv('possesion_event_q2_rpi.csv', fileEncoding="UTF-8-BOM")
rpia <- rpi$RPIA
rpib <- rpi$RPIB
rpiduration <- rpi$duration
oppo <- read.csv('possesion_event_q2_oppo.csv', fileEncoding="UTF-8-BOM")
oppoa <- oppo$OPPOA
oppob <- oppo$OPPOB
oppoduration <- oppo$duration
shot_for <- read.csv('possesion_event_q2_for.csv', fileEncoding="UTF-8-BOM")
forchance <- matrix(shot_for$Chance)
shot_for <- shot_for$Shot_for
shot_against <- read.csv('possesion_event_q2_against.csv', fileEncoding="UTF-8-BOM")
againstchance <- matrix(shot_against$Chance)
shot_against <- shot_against$Shot_against

formatrix <- matrix(nrow = 18, ncol = 2)
f <- 0
...
againstmatrix <- matrix(nrow = 17, ncol = 2)
f <- 0
...
plot(formatrix[, 1], formatrix[, 2], xlab = 'time', ylab = 'chance')
